...
class NotesViewModel @Inject constructor(
    ...
    fun onEvent(event: NotesEvent){
        ...
        when(event){
            ...
            is NotesEvent.Order -> {
                if(event.noteOrder == state.value.order
                    && event.noteOrder.orderType == state.value.order.orderType) return
    ...
    private fun getNotes(noteOrder: NoteOrder) {
        ...
        getNotesJob = notesUseCases.getNotes(noteOrder).onEach {
            notes -> _state.value = state.value.copy(notes = notes, order = noteOrder)
